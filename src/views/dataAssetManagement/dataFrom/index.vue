          ...
          <el-card class="combined-panel">
            <div class="left-panel">
              ...
              <div v-for="option in options" :key="option.value" class="option-item"
                :class="{ 'selected': option.selected }" @click.stop="expandOption(option)">
                <el-checkbox v-model="checkedOptions" :label="option.value" @change="toggleMiddleOptions(option)"
                  :indeterminate="option.indeterminate" @click.stop>
                  {{ option.label }}
                </el-checkbox>
                <i class="el-icon-arrow-right expand-icon" @click.stop="expandOption(option)"></i>
              </div>
            </div>
...
export default {
      ...
      options: [
        {
          ...
          label: 'kms',
          children: [
            { value: 'kms_certificate', label: 'kms_certificate (100字段)' },
            { value: 'ksm_config', label: 'ksm_config (300字段)' },
  ...
  computed: {
    ...
    fieldCount() {
      return this.selectedItems.reduce((total, item) => total + parseInt(item.fieldCount), 0);
    }
  },
  created() {
    ...
    updateMiddleCheckAllState() {
      ...
      for (let i = 0; i < this.options.length; i++) {
        ...
        if (checkedChildren.length === 0) {
          // 没有子节点被选中
          this.checkedOptions = this.checkedOptions.filter(opt => opt !== option.value);
          option.indeterminate = false;
          option.selected = false; // 移除高亮
