<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px" >
      ...
      <el-form-item label="打码规则" prop="maskRule">
        <el-input
          v-model="queryParams.maskRule"
          placeholder="请输入打码规则"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:proxyRule:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:proxyRule:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:proxyRule:remove']"
        >删除</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="proxyRuleList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="打码规则" align="center" prop="maskRule" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px"  @submit.native.prevent>
        ...
        <el-form-item label="打码规则" prop="maskRule">
          <el-input v-model="form.maskRule" placeholder="请输入打码规则" />
        </el-form-item>
      </el-form>
...
<script>
import { listProxyRule, getProxyRule, delProxyRule, addProxyRule, updateProxyRule } from "@/api/system/proxyRule";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProxyRule(this.queryParams).then(response => {
        this.proxyRuleList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateProxyRule(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addProxyRule(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除脱敏规则编号为"' + ids + '"的数据项？').then(function() {
        return delProxyRule(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/proxyRule/export', {
        ...this.queryParams
      }, `proxyRule_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
